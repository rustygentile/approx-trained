...
def train(wid, func, opt, results_folder, save_all_models=False):
    ...
    for step in range(steps * intervals):
        ...
        if opt is not 'GD':
            ...
            for p in model.parameters():
                if p.requires_grad:
                    p.data -= lr * np.exp(-a * step) * p.grad
...
if __name__ == '__main__':
    ...
    functions = ['step', 'cusp', '2gaussian']
    widths = [10, 100, 1000]
    seeds = [0]
    optimizers = ['Adam', 'SGD', 'GD']

    conf = {'name': 'func-approx-trained-ar-optimizers',
            'fixed_parameters': {},
    ...
    m_org = Organizer(config=conf, exp_folder='./results',
                      container_name='func-approx-optimizers10')
    ...
    for test in m_org.run_tests():
        ...
        train(width, function, optimizer, test_folder, save_all_models=True)
        m_org.complete_test()
        gc.collect()
